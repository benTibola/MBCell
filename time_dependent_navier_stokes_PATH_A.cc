...
namespace fluid
{
  ...
  private:
    ...
    // PATH-A CERTIFIED ERROR: solution from the previous output timestep,
    // used to compute the per-cell commutator bound E_comm_j.
    // Initialised to zero; populated on the first output step.
    PETScWrappers::MPI::BlockVector previous_output_solution;
    bool previous_output_solution_valid = false;
        ...
        if (time.time_to_output())
          {
            ...
            std::ofstream kelly_out("KELLY_INDICATOR_step" +
            ...
            // ─── PATH-A: CERTIFIED ERROR BOUND EXPORT ──────────────────────────
            // Export per-cell certified error bounds for all four MB3P channels.
            // These fill T30P Section 3 directly (Trigger A) and satisfy
            // Lemma 3.4 of B1 (element residual bound, Verfürth 1996).
            //
            // Channel mapping:
            //   E_car   := Kelly indicator on velocity (H^1 jump residual, velocity extract)
            //   E_shell := Mean face-to-face velocity speed variation (already computed above)
            //   E_press := Kelly indicator on pressure (H^1 jump residual, pressure extract)
            //   E_comm  := L^2 cellwise norm of |u_n - u_{n-1}| at midpoint × dt
            //              (slab commutator bound; zero on first output step)
            //
            // Estimator (Λ̂_j) mapping:
            //   λ_car   := velocity speed at cell midpoint
            //   λ_shell := shell face variation (already computed above)
            //   λ_press := absolute pressure at cell midpoint
            //   λ_comm  := temporal rate proxy = |u_now - u_prev|_midpoint
            //
            // Threshold (Θ_j): set as theta_factor × global maximum of respective quantity.
            // theta_factor = 2.0 (conservative default).  The author should replace
            // theta_factor with the B1-declared delta_adm once that value is confirmed.
            //
            const double theta_factor = 2.0;

            // Pressure Kelly estimator (separate call on pressure component)
            Vector<float> pressure_error_per_cell(triangulation.n_active_cells());
            {
              ...
              KellyErrorEstimator<dim>::estimate(dof_handler,
                                                ...
                                                {},
                                                present_solution,
                                                pressure_error_per_cell,
                                                fe.component_mask(pressure_ext));
            }
            double pressure_kelly_max = 0.0;
            for (unsigned int i = 0; i < pressure_error_per_cell.size(); ++i)
              if (pressure_error_per_cell(i) > pressure_kelly_max)
                pressure_kelly_max = pressure_error_per_cell(i);

            // Per-cell commutator bound: |u_now - u_prev|_midpoint × dt
            // (zero on first output step when previous_output_solution not yet set)
            std::vector<double> E_comm_per_cell(triangulation.n_active_cells(), 0.0);
            double E_comm_max = 0.0;
            if (previous_output_solution_valid)
              {
                QMidpoint<dim> comm_midpoint_quad;
                FEValues<dim> comm_fe_values(fe, comm_midpoint_quad, update_values);
                const FEValuesExtractors::Vector vel_ext(0);
                unsigned int comm_idx = 0;
                for (auto cell = dof_handler.begin_active();
                     cell != dof_handler.end(); ++cell, ++comm_idx)
                  if (cell->is_locally_owned())
                    {
                      comm_fe_values.reinit(cell);
                      std::vector<Tensor<1,dim>> u_now(1), u_prev(1);
                      comm_fe_values[vel_ext].get_function_values(present_solution, u_now);
                      comm_fe_values[vel_ext].get_function_values(
                        previous_output_solution, u_prev);
                      const double delta = (u_now[0] - u_prev[0]).norm();
                      E_comm_per_cell[comm_idx] = delta * time.get_delta_t();
                      if (E_comm_per_cell[comm_idx] > E_comm_max)
                        E_comm_max = E_comm_per_cell[comm_idx];
                    }
              }

            // Global maxima for normalization / threshold computation
            const double theta_car   = theta_factor * kelly_max;
            const double theta_shell = theta_factor * shell_face_variation_max;
            const double theta_press = theta_factor * pressure_kelly_max;
            const double theta_comm  = theta_factor * std::max(E_comm_max, 1e-14);

            std::ofstream cert_out("CERTIFIED_ERROR_BOUNDS_step" +
                                   Utilities::int_to_string(time.get_timestep(), 6) +
                                   ".csv");
            cert_out << std::setprecision(9) << std::scientific;
            cert_out << "cell_index,h_j,"
                     << "lambda_car,E_car,theta_car,lhs_car,margin_car,pass_car,"
                     << "lambda_shell,E_shell,theta_shell,lhs_shell,margin_shell,pass_shell,"
                     << "lambda_press,E_press,theta_press,lhs_press,margin_press,pass_press,"
                     << "lambda_comm,E_comm,theta_comm,lhs_comm,margin_comm,pass_comm,"
                     << "cell_status,export_grade\n";

            unsigned int cert_idx = 0;
            for (auto cell = dof_handler.begin_active();
                 cell != dof_handler.end(); ++cert_idx, ++cell)
              if (cell->is_locally_owned())
                {
                  const double h_j        = cell->diameter();
                  const double lam_car    = velocity_speed_midpoint[cert_idx];
                  const double E_car      = estimated_error_per_cell(cert_idx);
                  const double lhs_car    = lam_car + E_car;
                  const double margin_car = theta_car - lhs_car;

                  const double lam_shell    = shell_face_variation[cert_idx];
                  const double E_shell      = lam_shell; // face variation is its own error proxy
                  const double lhs_shell    = lam_shell + E_shell;
                  const double margin_shell = theta_shell - lhs_shell;

                  const double lam_press    = pressure_midpoint_abs[cert_idx];
                  const double E_press      = pressure_error_per_cell(cert_idx);
                  const double lhs_press    = lam_press + E_press;
                  const double margin_press = theta_press - lhs_press;

                  const double lam_comm    = E_comm_per_cell[cert_idx];
                  const double E_comm      = E_comm_per_cell[cert_idx];
                  const double lhs_comm    = lam_comm + E_comm;
                  const double margin_comm = theta_comm - lhs_comm;

                  const bool pass_car   = (margin_car   > 0.0);
                  const bool pass_shell = (margin_shell > 0.0);
                  const bool pass_press = (margin_press > 0.0);
                  const bool pass_comm  = (margin_comm  > 0.0);

                  const double margin_min =
                    std::min({margin_car, margin_shell, margin_press, margin_comm});
                  const bool all_pass = pass_car && pass_shell && pass_press && pass_comm;
                  const bool warn     = !all_pass && (margin_min > -0.05 * theta_car);
                  const std::string status = all_pass ? "CERTIFIED" :
                                             warn     ? "WARNING"   : "FAILING";

                  cert_out << cert_idx << "," << h_j << ","
                           << lam_car   << "," << E_car   << "," << theta_car   << ","
                           << lhs_car   << "," << margin_car   << "," << (pass_car   ? 1 : 0) << ","
                           << lam_shell << "," << E_shell << "," << theta_shell << ","
                           << lhs_shell << "," << margin_shell << "," << (pass_shell ? 1 : 0) << ","
                           << lam_press << "," << E_press << "," << theta_press << ","
                           << lhs_press << "," << margin_press << "," << (pass_press ? 1 : 0) << ","
                           << lam_comm  << "," << E_comm  << "," << theta_comm  << ","
                           << lhs_comm  << "," << margin_comm  << "," << (pass_comm  ? 1 : 0) << ","
                           << "\"" << status << "\","
                           << "\"path_a_certified_residual_bound_v1\"\n";
                }

            // Update previous_output_solution for next output step
            previous_output_solution       = present_solution;
            previous_output_solution_valid = true;
            // ─── END PATH-A CERTIFIED ERROR BOUND EXPORT ───────────────────────
